...
import { listAdminEducation } from "@/lib/admin/education/queries";
...
export default async function AdminEducationPage() {
  ...
  const { data, error } = await listAdminEducation(auth.supabase);
  ...
  return (
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="font-serif text-2xl text-ink">All education</h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-ink-soft">
            Education rows are stored as credentials with kind{" "}
            <code>degree</code>. Drafts stay in the admin. Public pages still
      ...
      {records.length === 0 && !error ? (
        <p className="mt-8 rounded-xl border border-dashed border-line bg-paper-elevated p-6 text-sm text-ink-soft">
          No education records in Supabase yet. Create one here. Do not load
          real school or degree history in this step.
        </p>
      ) : (
        <div className="mt-8 overflow-x-auto rounded-xl border border-line bg-paper-elevated">
          <table className="min-w-full text-left text-sm">
            <caption className="sr-only">Education</caption>
              ...
              {records.map((record) => (
                ...
                >
                  <td className="px-4 py-3">
                    <Link
                      href={`/admin/education/${record.id}`}

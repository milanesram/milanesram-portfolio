...
import { listAdminTraining } from "@/lib/admin/training/queries";
...
export default async function AdminTrainingPage() {
  ...
  const { data, error } = await listAdminTraining(auth.supabase);
  ...
  return (
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="font-serif text-2xl text-ink">All training</h2>
          <p className="mt-2 max-w-2xl text-sm leading-6 text-ink-soft">
            Training rows are stored as credentials with kind{" "}
            <code>training</code>. Drafts stay in the admin. Public pages still
      ...
      {records.length === 0 && !error ? (
        <p className="mt-8 rounded-xl border border-dashed border-line bg-paper-elevated p-6 text-sm text-ink-soft">
          No training records in Supabase yet. Create one here. Do not load
          real training names or verification data in this step.
        </p>
      ) : (
        <div className="mt-8 overflow-x-auto rounded-xl border border-line bg-paper-elevated">
          <table className="min-w-full text-left text-sm">
            <caption className="sr-only">Training</caption>

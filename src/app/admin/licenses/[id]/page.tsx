...
import {
  LicenseForm,
  DeleteLicenseButton,
} from "@/components/admin/LicenseForm";
import { StatusBadge } from "@/components/admin/StatusBadge";
import { requireAdminMutation } from "@/lib/admin/authorization";
import { isUuid } from "@/lib/admin/ids";
import { getAdminLicense } from "@/lib/admin/licenses/queries";
...
export default async function EditLicensePage({
  ...
  const result = await getAdminLicense(auth.supabase, id);
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="max-w-2xl rounded-xl border border-line bg-paper-elevated p-6">
        <h3 className="font-serif text-xl text-ink">License details</h3>
        <div className="mt-6 space-y-5">
          <LicenseForm license={result.data} />
          <DeleteLicenseButton

...
/**
 * Public About reads hosted `about_page` plus published Journey
 * milestones. Education glance remains a temporary static credential
 * dependency until Step 52F.
 */
...
async function loadPublishedAboutPage(): Promise<PublishedAboutPageResult> {
  ...
  const [aboutResult, milestoneResult] = await Promise.all([
    supabase
      .from("about_page")
      .select(
        `${ABOUT_COLUMNS}, paragraphs:about_page_paragraphs(id, body, sort_order), list_items:about_page_list_items(id, kind, body, sort_order)`,
      )
  ...
  const mediaResult =
    mediaIds.length > 0
      ? await supabase.from("media_assets").select(MEDIA_COLUMNS).in("id", mediaIds)
      : { data: [], error: null };

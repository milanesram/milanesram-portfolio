...
export type ParsedEducationInput = {
  id: string | null;
  name: string;
  issuer: string;
...
function parseIntent(formData: FormData): ParseResult<EducationIntent> {
  ...
  return { ok: true, value: raw as EducationIntent };
}
...
export function parseEducationFormData(
  formData: FormData,
): ParseResult<ParsedEducationInput> {
  const idRaw = readString(formData, "id");
  const id = idRaw && idRaw.length > 0 ? readUuid(idRaw) : null;

  if (idRaw && idRaw.length > 0 && !id) {
    return { ok: false, error: "That education record could not be saved." };
  }

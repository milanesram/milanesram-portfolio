...
export type ParsedCertificationInput = {
  id: string | null;
  name: string;
  issuer: string;
...
function parseIntent(formData: FormData): ParseResult<CertificationIntent> {
  ...
  return { ok: true, value: raw as CertificationIntent };
}
...
export function parseCertificationFormData(
  formData: FormData,
): ParseResult<ParsedCertificationInput> {
  const idRaw = readString(formData, "id");
  const id = idRaw && idRaw.length > 0 ? readUuid(idRaw) : null;

  if (idRaw && idRaw.length > 0 && !id) {
    return { ok: false, error: "That certification could not be saved." };
  }

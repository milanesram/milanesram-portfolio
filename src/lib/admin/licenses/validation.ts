...
export type ParsedLicenseInput = {
  id: string | null;
  name: string;
  issuer: string;
...
function parseIntent(formData: FormData): ParseResult<LicenseIntent> {
  ...
  return { ok: true, value: raw as LicenseIntent };
}
...
export function parseLicenseFormData(
  formData: FormData,
): ParseResult<ParsedLicenseInput> {
  const idRaw = readString(formData, "id");
  const id = idRaw && idRaw.length > 0 ? readUuid(idRaw) : null;

  if (idRaw && idRaw.length > 0 && !id) {
    return { ok: false, error: "That license could not be saved." };
  }

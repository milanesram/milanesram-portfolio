    ...
    >
      ...
      <label className={labelClass}>
        ...
        <input
          name="year_label"
          maxLength={40}
          defaultValue={education?.year_label ?? ""}
          disabled={pending}
          className={fieldClass}
        ...
        >
          Display string only. The schema has no start date, end date, GPA, or
          honors field.
        </span>
      </label>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <label className={labelClass}>
          ...
          <select
            name="track"
            defaultValue={education?.track ?? "all"}
            disabled={pending}
            className={fieldClass}
          >
      ...
      <label className="flex min-h-11 items-center gap-3 text-sm font-medium text-ink">
        <input
          type="checkbox"
          name="highlight"
          defaultChecked={education?.highlight}
          disabled={pending}
      ...
      <label className="flex min-h-11 items-center gap-3 text-sm font-medium text-ink">
        <input
          ...
          defaultChecked={education?.needs_verification}
          disabled={pending}
          className="size-4 accent-[var(--accent)]"
        />
        Needs verification
  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        if (
          !window.confirm(
            `Delete the education record “${name}”? This cannot be undone from the admin UI.`,
          )
        ) {
          event.preventDefault();
        }

    ...
    >
      ...
      <label className={labelClass}>
        ...
        <input
          name="year_label"
          maxLength={40}
          defaultValue={training?.year_label ?? ""}
          disabled={pending}
          className={fieldClass}
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <label className={labelClass}>
          ...
          <select
            name="track"
            defaultValue={training?.track ?? "all"}
            disabled={pending}
            className={fieldClass}
          >
        ...
        <label className={labelClass}>
          ...
          <input
            ...
            min={0}
            max={9999}
            defaultValue={training?.sort_order ?? 0}
            disabled={pending}
            className={fieldClass}
          />
        </label>
      ...
      <label className="flex min-h-11 items-center gap-3 text-sm font-medium text-ink">
        <input
          type="checkbox"
          name="highlight"
          defaultChecked={training?.highlight}
          disabled={pending}
      ...
      <label className="flex min-h-11 items-center gap-3 text-sm font-medium text-ink">
        <input
          ...
          defaultChecked={training?.needs_verification}
          disabled={pending}
          className="size-4 accent-[var(--accent)]"
        />
        Needs verification
  ...
  return (
    <form
      ...
      onSubmit={(event) => {
        if (
          !window.confirm(
            `Delete the training “${name}”? This cannot be undone from the admin UI.`,
          )
        ) {
          event.preventDefault();
        }

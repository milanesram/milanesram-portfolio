...
export type ChromeMutationState = {
  error: string | null;
  message: string | null;
};

const initialState: ChromeMutationState = { error: null, message: null };
const fieldClass =
  "mt-2 min-h-11 w-full rounded-lg border border-line bg-paper px-3 text-ink disabled:opacity-60";
const labelClass = "block text-sm font-medium text-ink";

export type IndexChromePage = {
  id: string;
...
export function IndexChromeForm({
  page,
  additionalHeading,
  action,
}: {
  page?: IndexChromePage | null;
  additionalHeading?: boolean;
  action: (
    previous: ChromeMutationState,
    formData: FormData,
  ) => Promise<ChromeMutationState>;
}) {
  ...
  return (
    <form action={formAction} className="space-y-5" noValidate>
      ...
      {additionalHeading ? (
        <label className={labelClass}>
          Additional experience heading
          <input
            name="additional_heading"
            required
            defaultValue={page?.additional_heading}
            disabled={pending}
            className={fieldClass}
          />
        </label>
